...
//	CheckBox chk_pass;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		etUserName=(EditText) findViewById(R.id.editText1);//登录需要的新信息，etUserName和登录的用户名输入编辑框绑定，
		etUserPass=(EditText) findViewById(R.id.editText2);//登录需要的新信息，etUserPass和登录密码的输入编辑框绑定
		pref=getSharedPreferences("UserInfo", MODE_PRIVATE);//登录需要的新信息，在编辑存入用户和密码信息的时候会用到，我也不晓得是什么鬼
		editor=pref.edit();//登录需要的新信息，应该是启动编辑，maybe
		 ...
		 String name=pref.getString("userName", "");
		  ...
		  bt1.setOnClickListener(new MyClickListener(){
	    	  public void onClick(View v) {
					...
					String name =etUserName.getText().toString().trim();//登录需要的新信息，取用户名
	    	  ...
	    	  }//如果用户名等于admin，而且密码等于12345678，就运行startActivity(intent)；执行意图，即实现页面跳转，并且输出登录成功提示信息
	    	  });
